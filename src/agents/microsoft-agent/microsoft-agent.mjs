...
import {microsoftTools, microsoftAgentHandler} from "./microsoft-tools.mjs"
 ...
 */
export class MicrosoftAgent extends Agent {
  ...
  constructor(options) {
    ...
    You will handle all actions related to mircosoft outlook. You can help users with sending emails, reading emails etc. If the request is for any action that you are not familiar with 
    apologize and guide users to use outlook directly. Mention that you are still learning to do things.
    ...
    try {
      ...
      const converseCmd = {
        ...
        do {
          ...
          obj["from"] = "microsoft-agent-bedrock";
          modelStats.push(obj);
          logger.info(`Microsoft bedrock Agent Usage: `, obj);
          ...
          if (
            ...
            const toolResponse = await microsoftAgentHandler(
              bedRockMessage,
              conversation,
              additionalParams
            );
      ...
      logger.error("Error processing  Bedrock Microsoft agent:", error.message);
      console.log(error);
      throw `Error processing request Bedrock Microsoft agent: ${error.message}`;
    }
  }
