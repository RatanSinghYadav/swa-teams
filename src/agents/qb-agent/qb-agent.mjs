...
import {qbTools, qbAgentHandler} from "./qb-tools.mjs"
 ...
 */
export class QuickBooksAgent extends Agent {
  ...
  constructor(options) {
    ...
    You will handle all actions related to quick books. You can help users with bills, bill payment, vendors, purchases and so on. If the request is for any action that you are not familiar with 
    apologize and guide users to use the quick books tool. Mention that you are still learning to do things.
    ...
    Use your knowledge to figure out the required schema for making the quickbooks api calls.
    ...
    After fetching results, summarize the data for users, and add only the most relevant fields, ignoring empty ones.
    ...
    try {
      ...
      const converseCmd = {
        ...
        do {
          ...
          obj["from"] = "qb-agent-bedrock";
          modelStats.push(obj);
          logger.info(`qb bedrock Agent Usage: `, obj);
          ...
          if (
            ...
            const toolResponse = await qbAgentHandler(
              bedRockMessage,
              conversation,
              additionalParams
            );
      ...
      logger.error("Error processing  Bedrock QB agent:", error.message);
      console.log(error);
      throw `Error processing request Bedrock QB agent: ${error.message}`;
    }
  }
 ...
 async handleSingleResponse(input){
  ...
  enhanceSystemPrompt(additionalParams) {
    this.promptTemplate =
      this.promptTemplate +
      `\n. Current user is: ${additionalParams.email}. If no other email or user id is provided, use this.`;
  }
